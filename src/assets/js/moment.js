...
(function (global, factory) {
	...
	function isArray(input) {
		return Array.isArray(input) || Object.prototype.toString.call(input) === '[object Array]';
	}
	...
	function isValid(m) {
		if (m._isValid == null) {
			...
			const parsedParts = some$1.call(flags.parsedDateParts, i => {
				return i != null;
			});
	...
	const matchWord = /\d*['a-z\u00A0-\u05FF\u0700-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF]+|[\u0600-\u06FF\/]+(\s*?[\u0600-\u06FF]+){1,2}/i;
	...
	function configFromRFC2822(config) {
		let string; let match; let dayFormat;
		let dateFormat; let timeFormat; let tzFormat;
		const timezones = {
			' GMT': ' +0000',
			' EDT': ' -0400',
			' EST': ' -0500',
		...
		let timezone; let timezoneIndex;
			...
			switch (match[5].length) {
				case 2: // Military
					if (timezoneIndex === 0) {
                        ...
                        ((String(timezoneIndex)).replace(/^-?/, '0')).match(/..$/)[0] + '00';
					}
	...
	function dayOfYearFromWeekInfo(config) {
		let w; let weekYear; let week; let weekday; let dow; let doy; let temp; let weekdayOverflow;
	...
	function offsetFromString(matcher, string) {
		...
		const parts = (String(chunk)).match(chunkOffset) || ['-', 0, 0];
 ...
 */
	...
	function bubble() {
		...
		const data = this._data;
		let seconds; let minutes; let hours; let years; let monthsFromDays;
	...
	function toISOString$1() {
		...
		let minutes; let hours; let years;
