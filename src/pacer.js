...
const PACER = {
	...
	convertToCourtListenerCourt(pacer_court_id) {
	...
	isDocketDisplayUrl(url) {
		...
		const re = /servlet\/TransportRoom(?:\?servlet=([^?&]+)(?:[\/&#;].*)?)?$/;
		const match = url.match(re);
		if (match) {
			const servlet = match[1];
	...
	isSingleDocumentPage(url, document) {
		const inputs = document.querySelectorAll('input');
		const lastInput = inputs.length && inputs[inputs.length - 1].value;
		...
		const hasImageReceipt = Boolean($('td:contains(Image)').length);
	...
	getDocumentIdFromUrl(url) {
		const match = (url || '').match(/\/(?:doc1|docs1)\/(\d+)/);
		if (match) {
	...
	// form.
	getDocumentIdFromForm(url, document) {
		if (PACER.isDocumentUrl(url)) {
			...
			if (inputs.length && last_input.value === 'View Document') {
				...
				const onsubmit = last_input.form.getAttribute('onsubmit');
				const goDLS = PACER.parseGoDLSFunction(onsubmit);
	...
	getCaseNumberFromInputs(url, document) {
		if (PACER.isDocumentUrl(url)) {
			const inputs = document.querySelectorAll('input');
			const last_input = inputs[inputs.length - 1];
			if (inputs.length && last_input.value === 'Download All') {
				// Attachment page.
				const onclick = last_input.getAttribute('onclick');
				const match = onclick.match(/[?&]caseid=(\d+)/i);
				if (match && match[1] !== '0') {
					return match[1];
				}
	...
	parseGoDLSFunction(goDLS_string) {
	...
	hasPacerCookie(cookieString) {
		const cookies = {};
		cookieString.replace(/\s*([^=;]+)=([^;]*)/g, (match, name, value) => {
			cookies[name.trim()] = value.trim();
		});
		const pacerCookie = cookies.PacerUser || cookies.PacerSession;
		return Boolean(pacerCookie && !pacerCookie.match(/unvalidated/));
	},

	// Returns true if the given court identifier is for an appellate court.
	isAppellateCourt(court) {
		return Boolean(PACER.APPELLATE_COURTS[court]);
	},
