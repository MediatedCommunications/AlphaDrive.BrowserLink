...
const ContentDelegate = function (
	url,
	path,
	court,
	pacer_case_id,
...
ContentDelegate.prototype.checkRestrictions = function () {
	...
	for (const td of document.querySelectorAll('b')) {
		if (
...
ContentDelegate.prototype.findAndStorePacerDocIds = function () {
	...
	for (let i = 0; i < this.links.length; i++) {
		...
		if (PACER.isDocumentUrl(link.href)) {
			...
			const goDLS = PACER.parseGoDLSFunction(onclick);
	...
	chrome.storage.local.set(docsToCases, () => {
		console.info(
		...
		$('<div class="recap-banner"/>')
			.append(
				$('<a/>', {
					...
					.append(
						$('<img/>', {
							src: chrome.extension.getURL('assets/images/icon-0128.png')
						})
					)
...
ContentDelegate.prototype.handleDocketDisplayPage = function () {
	...
	const isDocketDisplayUrl = PACER.isDocketDisplayUrl(this.url);
	const isDocketHistoryDisplayUrl = PACER.isDocketHistoryDisplayUrl(this.url);
	if (!(isDocketHistoryDisplayUrl || isDocketDisplayUrl)) {
		// If it's not a docket display URL or a docket history URL, punt.
		return;
	}

	const isAppellate = PACER.isAppellateCourt(this.court);
	...
	chrome.storage.local.get(
		...
		items => {
			if (!items.options.recap_disabled) {
				const callback = $.proxy(function (ok) {
					if (ok) {
...
ContentDelegate.prototype.handleSingleDocumentPageCheck = function () {
	...
	const callback = $.proxy(api_results => {
		console.info(
		...
		$('<div class="recap-banner"/>')
			.append(
				$('<a/>', {
					...
					.append('View this document for free')
			)
			.appendTo($('form'));
	}, this);

	const cl_court = PACER.convertToCourtListenerCourt(this.court);
...
ContentDelegate.prototype.onDocumentViewSubmit = function (event) {
	...
	const originalSubmit = originalForm.getAttribute('onsubmit');
	...
	let document_number; let attachment_number; let docket_number;

	if (!PACER.isAppellateCourt(this.court)) {
		// This CSS selector duplicated in isSingleDocumentPage
		const image_string = $('td:contains(Image)').text();
		const regex = /(\d+)-(\d+)/;
		const matches = regex.exec(image_string);
		if (!matches) {
			form.submit();
			return;
		}
	...
	httpRequest(
		...
		(type, ab, xhr) => {
          ...
          xhr.statusText
			...
			const blob = new Blob([new Uint8Array(ab)], {type});
			...
			if (type === 'application/pdf') {
				...
				const html =
          '<style>body { margin: 0; } iframe { border: none; }' +
          '</style><iframe src="' +
		...
		(type, ab, xhr) => {
			...
			this.recap.getPacerCaseIdFromPacerDocId(
				...
				pacer_case_id => {
					...
					const displayPDF = function (items) {
						let filename;
                ...
                '.pdf';
						...
						let {external_pdf} = items.options;
						if (
              ...
              !navigator.plugins.namedItem('Chrome PDF Viewer')
                ...
                match[3];
							...
							documentElement.innerHTML =
                match[1] + '<p><iframe src="about:blank"' + match[3]; // Clear "Waiting..." message
						}
					}.bind(this);

					chrome.storage.local.get('options', displayPDF);

					const uploadDocument = function (items) {
		...
		$.proxy(function (api_results) {
          ...
          'attach links and icons where appropriate.'
			...
			for (let i = 0; i < this.links.length; i++) {
				...
				const recap_link = $('<a/>', {
					class: 'recap-inline',
					title: 'View this document for free',
					href
				});
				recap_link.click(
