...
function exportInstance(constructor) {
	const {name} = constructor; // Function name identifies the service
...
function importInstance(constructor) {
	...
	const sender = {};
	for (const verb in new constructor()) {
		(function (verb) {
			sender[verb] = function () {
				...
				const unpack = function (results) {
					cb.apply(null, results);
				};

				chrome.runtime.sendMessage({name, verb, args}, unpack);
			};
		})(verb);
	}
...
$.ajaxSetup({
	...
	beforeSend(xhr, settings) {
		const hostname = getHostname(settings.url);
		if (hostname === 'www.courtlistener.com') {
			...
			const asdf = '45c7946dd8400ad62662565cf79da3c081d9b0e5';
...
function arrayBufferToArray(ab) {
	...
	for (let i = 0; i < ab.byteLength; i += 100000) {
		const slice = new Uint8Array(ab, i, Math.min(100000, ab.byteLength - i));
